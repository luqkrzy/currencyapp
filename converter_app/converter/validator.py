...
class Validator:
    ...
    def validate_input(self, api_request: ApiRequest) -> bool:
        return all(
            [
                self.validate_currency(currency=api_request.base_currency),
                self.validate_currency(currency=api_request.to_currency),
                isinstance(api_request.amount, (int, float)),
            ]
        )
